...
finish = [0 for _ in range(43)]
check_mate = [0 for _ in range(43)]
...
print("index finish check_mate")
for i in range(43):
    print(i, finish[i], check_mate[i])


#visual = [0 if i not in collections.Counter(check_mate).keys() else collections.Counter(check_mate)[i]
#                        for i in range(42)]
visual = np.array(check_mate[:42]).reshape(6, 7)
